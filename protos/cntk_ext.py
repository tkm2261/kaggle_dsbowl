...
def get_data_id(data):
    ...
    for i in range(0, sample_image.shape[0] - 3):
        tmp = []
        for j in range(3):
            img = sample_image[i + j]
            img = 255.0 / np.amax(img) * img
            img = equalize_hist(img.astype(np.uint16))
...
def calc_features():
    ...
    for folder in glob.glob(STAGE1_FOLDER + '*'):
        ...
        pix_resampled = patient_pixels  # , spacing = resample(patient_pixels, patient_data, [1, 0.625, 0.625])
